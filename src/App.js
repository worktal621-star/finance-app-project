...
function exportCSV(salary, fixedExpenses, months) {
  const rows = [["חודש", "קטגוריה", "קטגוריית מערכת", "בפועל", "סוג"]];
  Object.keys(months).sort().forEach((ym) => {
    ...
    (months[ym].variableExpenses || []).forEach((e) => {
      rows.push([label, e.category, resolveSystemCategory(e.category), num(e.actual), "משתנה"]);
    });
    (months[ym].categories || []).forEach((c) => rows.push([label, c.name, "", num(c.spent), "קטגוריה (מעקב בלבד)"]));
  });
  const BOM = "\uFEFF";
...
const VariableExpenseRow = ({ item, onEdit, onDelete, onAddExpense }) => {
...
function generateInsights({ salaryNum, totalFixed, actualTotal, remainingActual, savingGoal, variableExpenses, store, activeMonth }) {
  const insights = [];
  ...
  // תחזית לסוף חודש
  if (salaryNum > 0) {
    const pct = (totalActual / salaryNum) * 100;
    if (remainingActual > 0) {
      ...
      insights.push({
        ...
        icon: "alert",
        color: "red",
        text: `ההוצאות עברו את ההכנסה ב-₪${Math.abs(remainingActual).toLocaleString()} החודש`,
      });
    }
  }
  ...
  if (prevAvgTotal !== null && actualTotal > 0) {
    ...
    if (diff > 0 && diffPct > 10) {
      insights.push({
        ...
        icon: "trending-up",
        color: "red",
        text: `ההוצאות המשתנות גבוהות ב-${diffPct}% לעומת הממוצע האחרון`,
      });
      ...
      insights.push({
        ...
        icon: "trending-down",
        color: "emerald",
        text: `הצלחת לצמצם הוצאות ב-${diffPct}% לעומת הממוצע — כל הכבוד!`,
      });
    }
  }
  ...
  if (variableExpenses.length > 0) {
    ...
    if (num(top.actual) > 0) {
      ...
      insights.push({
        ...
        icon: "pie",
        color: "violet",
        text: `${top.category} היא ההוצאה הגבוהה ביותר — ₪${num(top.actual).toLocaleString()} (${topPct}% מסך ההוצאות)`,
      });
    }
  }
  ...
  if (savingGoalNum > 0 && salaryNum > 0) {
    if (remainingActual >= savingGoalNum) {
      insights.push({
        ...
        icon: "piggy",
        color: "emerald",
        text: `נראה שתעמוד ביעד החיסכון של ₪${savingGoalNum.toLocaleString()} החודש`,
      });
  ...
  if (variableExpenses.length > 0 && actualTotal > 0) {
    ...
    if (saving10 > 0) {
      insights.push({
        ...
        icon: "lightbulb",
        color: "indigo",
        text: `צמצום של 10% ב${top.category} יחסוך לך ₪${saving10.toLocaleString()} בחודש`,
      });
    }
  }
...
const AIInsights = ({ salaryNum, totalFixed, actualTotal, remainingActual, savingGoal, variableExpenses, store, activeMonth }) => {
  const insights = useMemo(
    () => generateInsights({ salaryNum, totalFixed, actualTotal, remainingActual, savingGoal, variableExpenses, store, activeMonth }),
    [salaryNum, totalFixed, actualTotal, remainingActual, savingGoal, variableExpenses, store, activeMonth]
  );
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
      ...
      <div className="p-4 space-y-2.5">
        {insights.map((insight) => {
          ...
          return (
            <div key={insight.id} className={`flex items-start gap-3 p-3.5 rounded-xl border ${style.bg} ${style.border} transition-all hover:shadow-sm`}>
...
const HomeScreen = ({ salaryNum, totalFixed, actualTotal, remainingActual, actualPct, remainingAfterExpenses, remainingAfterGoal, savingGoal, savingGoalPct, savingsPct, categories, variableExpenses, fixedExpenses, isCurrentMonth, activeMonth, currentMonth, editingSalary, salaryDraft, setSalaryDraft, openSalaryEdit, commitSalary, setEditingSalary, editingGoal, goalDraft, setGoalDraft, openGoalEdit, commitGoal, setEditingGoal, setActiveTab, store }) => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Quick stats — הוצאות קבועות + בפועל בלבד */}
...
// ── ExpensesScreen ──
const ExpensesScreen = ({ variableExpenses, editVariable, deleteVariable, addExpenseToVariable, openAddVar, activeMonth }) => {
            ...
            <div className="relative w-32 h-32 flex-shrink-0">
              <svg viewBox="-1 -1 2 2" className="w-full h-full -rotate-90">
                {chartData.map((slice, i) => {
                  ...
                  return (
                    <path key={i} d={`M ${sx} ${sy} A 1 1 0 ${slice.percent > 50 ? 1 : 0} 1 ${ex} ${ey} L 0 0`} fill={slice.color} />
                  );
                })}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-right">
            ...
            <tbody className="divide-y divide-slate-100">
              {variableExpenses.length === 0 ? (
                <tr><td colSpan={3} className="px-5 py-8 text-center text-slate-400 text-sm">אין הוצאות משתנות — לחץ + להוספה</td></tr>
        ...
        {variableExpenses.length > 0 && (
          <div className="px-5 py-3 bg-slate-50/60 border-t border-slate-100 flex justify-between text-sm font-bold text-slate-700">
            ...
            <span>₪{totalAct.toLocaleString()}</span>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="divide-y divide-slate-100">
          {sortedMonths.length === 0 ? (
            ...
            const mRem = salaryNum - (totalFixed + mVarAct);
            ...
            return (
              <button key={ym} onClick={() => setActiveMonth(ym)} className={`w-full flex items-center px-5 py-4 transition-colors ${isActive ? "bg-indigo-50" : "hover:bg-slate-50"}`}>
                <div className="flex-1 text-right">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">הוצאות בפועל: ₪{(totalFixed + mVarAct).toLocaleString()}</p>
                </div>
...
export default function App() {
  ...
  const [addVarForm, setAddVarForm] = useState({ category: "", actual: "", description: "" });
  const openAddVar = () => { setAddVarForm({ category: "", actual: "", description: "" }); setAddVarModal(true); };
  const commitAddVar = () => {
    if (!addVarForm.category.trim()) return;
    const auto = calcAutoEstimate({ category: addVarForm.category, estimatedManual: false }, store.months, activeMonth);
    setMonthData({
      variableExpenses: [...variableExpenses, {
        id: Date.now(),
  ...
  const sharedProps = {
    activeMonth, currentMonth, isCurrentMonth, isPastMonth,
    salaryNum, totalFixed, actualTotal, remainingActual, actualPct,
    remainingAfterExpenses, remainingAfterGoal,
    savingGoal, savingGoalPct, savingsPct,
      ...
      <main className="max-w-lg mx-auto px-4 pt-5">
        ...
        {activeTab === "expenses" && (
          <ExpensesScreen
            ...
            openAddVar={openAddVar} activeMonth={activeMonth}
          />
        )}
        {activeTab === "history" && (
          <HistoryScreen
